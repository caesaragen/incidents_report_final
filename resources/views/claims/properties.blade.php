...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white dark:text-gray-200 leading-tight">
            {{ __('Property Destruction Claims') }}
        </h2>
    </x-slot>
...
<script type="text/javascript">
    $(function() {
        var table = $('.data-table').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{{ route('properties.index') }}", // Adjusted to point to the new method for PropertyDamage
            columns: [
                ...
                { data: 'date_of_incident', name: 'date_of_incident' },
                {
                    ...
                    render: function(data, type, row) {
                        return '<a href="{{ route("claims.property", ["claim_id" => "PLACEHOLDER"]) }}" target="_blank" class="edit btn btn-primary btn-sm">View Claim</a>'.replace('PLACEHOLDER', row.id);
                    }
                }
            ]
        });
