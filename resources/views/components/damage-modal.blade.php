<x-modal name="damage-form-modal" :show="$errors->isNotEmpty()" focusable>
    <form method="post" action="{{ route('damage.attachments.store') }}"class="p-6"enctype="multipart/form-data">
        @csrf
        <div class="flex flex-col">
            <h2 class="text-lg font-medium text-gray-900 dark:text-white">
                {{ __('Add Attachments') }}
            </h2>
            <input hidden value="{{$property_damages_id}}" name="property_damages_id"/>
            <div class="flex flex-wrap -mx-3 mb-6 gap-2 justify-between">
            <div class="mb-6 mx-2">
                <x-input-label for="agricultural_officer" class="mb-2">{{ __('Valuation officer (agricultural/livestock officer report)') }}</x-input-label>
